    ...
    def show(self):
        while self.running:
            ...
            try:
                ...
                filtered = filter_stats_snapshot(snap)
                log.info(f"STATS: {filtered}")
...
def filter_stats_snapshot(snap: dict) -> dict:
    """Filter internal/noisy keys from a stats snapshot for logging.

    Strips per-process bookkeeping and per-tile timing keys, and converts
    active_tiles dicts (large) to counts.
    """
    def _is_internal(k):
        return (
            (isinstance(k, str) and (
                ...
                k.startswith('partial_mm_time_total_ms:') or
                k.startswith('lock:')
