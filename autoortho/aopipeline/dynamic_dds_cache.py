...
class DynamicDDSCache:
    ...
    def __init__(self, cache_dir: str, max_size_mb: int = 4096, enabled: bool = True):
        ...
        # Budget manager reference: set by getortho.py after both are initialized
    ...
    def _filter_healable_missing(missing_indices, tile, max_zoom):
        """Remove permanently unavailable chunks from missing_indices.

        Chunks with permanent_failure=True (e.g. Bing PNG placeholders) have
        no source data and will never be healable. Excluding them from
        needs_healing prevents the healing system retrying them every session.
        """
        ...
        if not chunks:
            return missing_indices
        return [i for i in missing_indices
